...
stones = ["stone.png", "pebble.png", "big_stone.png", "rock.png", "shuriken.png", "cold_shuriken.png"]
thresholds = {"stone.png": 0.7, "pebble.png":0.48, "big_stone.png": 0.6, "rock.png": 0.8, "shuriken.png": 0.75, "cold_shuriken.png": 0.64}
...
for stone in stones:
	...
	for pt in range(0, len(points)-1, 2):
		if pt+1 <= len(points):
			...
			pyautogui.moveTo(points[pt][0]+25, points[pt][1]+25)
			#pyautogui.click()
			#pyautogui.mouseDown()
			#time.sleep(2)
			print("Moved to point: ", points[pt+1])
			#pyautogui.moveTo(points[pt+1][0]+5, points[pt+1][1]+5)
			pyautogui.dragTo(points[pt+1][0]+25,points[pt+1][1]+25, 2, pyautogui.easeInQuad,button='left')
			#pyautogui.mouseUp()
